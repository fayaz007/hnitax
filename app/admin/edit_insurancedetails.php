                    ...
                    <form id="insuranceForm" class="custom-form" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="card curve-card card-dark">
                            ...
                            <div class="card-body">
                                ...
                                if ($user_id) {
                                    $query = "SELECT * FROM documents WHERE user_id = ? AND document_type = ? AND tax_year = ?";
                                    if ($stmt = $conn->prepare($query)) {
                                        $stmt->bind_param('iss', $user_id, $document_type, $tax_year); // Bind user_id and document_type to query
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
