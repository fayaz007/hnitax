...
try {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare($query);
        $stmt->bind_param('ssssssssii', $sold_stocks, $interest_income, $dividend_income, $rental_income, 
                          $ira_distributions, $foreign_income, $foreign_income_nature, $hsa_distributions, 
                          $user_id, $tax_year);
        $stmt->execute();
    } else {
        ...
        $query = "INSERT INTO other_income (user_id, tax_year, sold_stocks, interest_income, dividend_income, 
                  ...
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param('iissssssss', $user_id, $tax_year, $sold_stocks, $interest_income, $dividend_income, 
                          $rental_income, $ira_distributions, $foreign_income, $foreign_income_nature, $hsa_distributions);
        $stmt->execute();
    }
